			...
			<PopoverContent className="z-30 rounded-3xl p-6 w-72 bottom-4 left-8 shadow-2xl border border-border bg-card">
				<div className="flex flex-col gap-4 p-2">
					...
					<div className="flex gap-2 justify-end mt-4">
						<Button ripple size="sm" variant="secondary" onClick={close} className="h-9 px-4 rounded-full">
							Cancelar
						</Button>
						<Button ripple size="sm" variant="destructive" onClick={() => signOut()} className="h-9 px-4 rounded-full">
							Cerrar sesión
						</Button>
					</div>
				</div>
